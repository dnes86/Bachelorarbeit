...
if [ ! -f $buildout_cfg_file ]; then
    ...
    DOMAIN=$(awk '/\[domains\]/, /^$/' "$buildout_cfg_file" | sed -n 's/main = //p')
    ADHOCRACY_PORT=$(awk '/\[ports\]/, /^$/' "$buildout_cfg_file" | sed -n 's/main = //p')
    memcached_port=$(awk '/\[ports\]/, /^$/' "$buildout_cfg_file" | sed -n 's/memcached = //p')
    redis_port=$(awk '/\[ports\]/, /^$/' "$buildout_cfg_file" | sed -n 's/redis = //p')
    solr_port=$(awk '/\[ports\]/, /^$/' "$buildout_cfg_file" | sed -n 's/solr = //p')
    supervisor_port=$(awk '/\[ports\]/, /^$/' "$buildout_cfg_file" | sed -n 's/supervisor = //p')
    #
